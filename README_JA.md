...
*   **フル権限（YOLOモード）による完全な自律性**: サンドボックスを排除した特権モードで動作します。AIが物理ディスクのファイルを直接編集したり、コマンドを実行したりすることを「確認なし」で自律的に行えるため、真の自動化を実現します。
*   **安全かつポータブルなパス解決**: Node.js の `os.homedir()` を使用することで、Windows / macOS / Linux 問わず、実行ユーザーのホームディレクトリを自動的にコンテキスト（信頼されたパス）に含めます。環境構築の手間を最小限に抑えつつ、最大限のアクセス権限を提供します。
...
*   **独立した安全な隔離環境**: 実行時に独自のセッションと分離されたテンポラリ環境（`GEMINI_CLI_HOME`）を構築し、openclaw側で許可されたスキルのみを安全に連携させます。
...
> [!NOTE]
> インストーラーはインストール時点の**最新安定版**の OpenClaw と Gemini CLI をダウンロードします。
> このアダプタが使用するコア API は安定しているため、新しいバージョンでも問題なく動作するはずです。
> 問題が発生した場合は、上記のバージョンにダウングレードすることで、検証済みの環境を再現できます。
