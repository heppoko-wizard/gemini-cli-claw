...
## 1. 実行の同期的ロックとタイムアウト問題

**対象モジュール:** `adapter.js`

現在の実装では、Gemini CLIを呼び出す際に Node.js の `spawnSync`（同期実行処理）を用いています。

*   **影響:** 
    Gemini CLI の思考・実行プロセスが完全に終了し、最終的な結果がプロセスから返却されるまで、OpenClaw側（CLI、Telegram、Discordなどのフロントエンド）には一切の反応（文字のストリーミング等）が返りません。ユーザーからは「システムがフリーズしている」ように見えます。
*   **致命的リスク:**
    無限ループを防ぐためのセーフティとして、ハードコードで `GEMINI_TIMEOUT_MS = 120_000`（2分間）のタイムアウトが設定されています。そのため、Geminiがツール（ブラウザ操作など）を使って長時間推論やタスクを実行しようとした場合、途中でプロセスごと強制終了されクラッシュします。
*   **解決方針案:**
    `spawnSync` を `spawn`（非同期処理）に書き換え、Geminiの標準出力 (`stdout`/`stderr`) を逐次読み取りながらOpenClaw側にストリーミングで中継するアーキテクチャへの刷新が必要です。

## 2. コンテキスト剪定（Pruning）の完全な乖離

**対象モジュール:** `adapter.js` (セッション管理) および Gemini CLI本体
