...
async function main() {
        ...
        // Try to fetch the latest stable release from GitHub API
            ...
            if (downloadUrl && releaseTag) {
                ...
                if (dlRes.status !== 0) {
                    ...
                    const unzipRes = runCommand(`powershell -Command "Expand-Archive -Path '${zipPath}' -DestinationPath '${tmpExtractDir}' -Force"`, SCRIPT_DIR);
    ...
    if (!hasValidCredentials()) {
        ...
        if (!geminiSettings.security.auth?.selectedType) {
            ...
            geminiSettings.security.auth.enforcedType = 'oauth-personal';
            settingsChanged = true;
        }
        if (!geminiSettings.security.folderTrust || geminiSettings.security.folderTrust.enabled !== false) {
