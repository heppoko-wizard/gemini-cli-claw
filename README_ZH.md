...
本适配器采用**“便携式（自包含）”**设计。它不会将文件复制到系统的公共目录中，也不会污染您的全局系统。

*   **安装位置**：您解压 ZIP 包（或执行 `git clone`）的**此文件夹本身**即为安装和运行目录。
*   **运行机制**：安装脚本 (`setup.js`) 仅在该文件夹内本地安装依赖项，并将该文件夹的**绝对路径**作为快捷方式注册到 OpenClaw 的配置文件 (`~/.openclaw/openclaw.json`) 中。
*   **移动与卸载**：如果您移动了文件夹，只需再次运行 `setup.js` 即可更新路径。若要卸载，只需删除整个文件夹并从 `openclaw.json` 中移除相关引用即可，除了 Gemini CLI 的认证凭据 (`~/.gemini`) 之外，系统不会留下任何残留。
